...
while [ $COUNTER -lt $CLIENTS ]; do
	...
	java -Djava.security.policy=test/client.policy -classpath .:../server:../middleware test.DistributedResponseTimeEvaluation $SERVERNAME $PORT $TXNSPERCLIENT $SECONDS $COUNTER & 
