                        ...
                        if (waitObj.getLockType() == LockManager.WRITE) {
                            if (j == 0) {
                                ...
                                if (vect1.size () == 0) {
                                    ...
                                    // cannot be granted.
                                    // UNLESS the SAME transaction has a READ
                                    // lock on it, in which case we CONVERT it!
                                    if(vect1.size() == 1) {
                                        TrxnObj trxnObj2 = (TrxnObj) vect1.elementAt(0);
                                        if(trxnObj2.getXId() == waitObj.getXId()) {
                                            this.waitTable.remove(waitObj);
                                            try {
                                                synchronized (waitObj.getThread()) {
                                                    waitObj.getThread().notify();
            ...
            if (dataObj.getXId() == dataObj2.getXId()) {    
                ...
                if (dataObj.getLockType() == DataObj.READ) {    
                    ...
                    if (dataObj2.getLockType() == DataObj.WRITE) {    
                        ...
                        bitset.set(0, true);
                        System.out.println("Want WRITE, already have READ; CONVERT");
                    }
                }
            } 
            else {
